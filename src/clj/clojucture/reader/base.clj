...
(defn parsing-dates [ x ]
  (m/match x
           ...
           (as-> (re-matches #"(\S+)M,(\S+)" ,x) [ _ s ed]
                 (u/gen-dates-range
                   (jt/local-date s) (jt/months 1) (jt/local-date ed)))
           ...
           #"\d{4}\-\d{2}\-\d{2}\w{1,2}[+-]\d+WD,\d{4}\-\d{2}\-\d{2}"
           (as-> (re-matches #"(\d{4}\-\d{2}\-\d{2}\w+)([-+])(\d+)WD,(\S+)" x) [ _ s adj days ed ]
           ...
           (as-> (re-matches #"(\S+)Q" x ) [ _ s ]
                 (u/gen-dates-range (jt/local-date s) (jt/months 3) (jt/local-date s)) )
           :else nil )
  )